...
def test_failed_to_register_new_user_duplicate_name(client):
    response = register_or_login_user('user1', 'pass1', 'register', client)
...
def test_failed_login__with_invalid_username(client):
    response = register_or_login_user('user1111', 'pass1', 'login', client)
    assert response.status_code == 400
    assert response.get_json() == {'message': 'Invalid username or password'}


def test_failed_login_with_invalid_password(client):
